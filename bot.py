...
# ====================
# RENTAL CAR FLOW - AMHARIC (SIMPLIFIED)
# ====================
...
async def get_rental_region(message: types.Message, state: FSMContext):
    ...
    # Skip condition for rental and go directly to phone
    await ask_for_phone(message, state)
...
async def ask_for_phone(message: types.Message, state: FSMContext):
    ...
    if car_type == 'rental':
        # For rental, set condition to empty since we don't ask for it
...
async def get_phone(message: types.Message, state: FSMContext):
    ...
    if car_type == 'sale':
        # For sale, we already asked condition
        photo_prompt = "*ቀጣይ ደረጃ (አማራጭ)፡* የመኪናውን ፎቶዎች ይላኩ፡\n\n"
        photo_prompt += "• ፊት ለፊት እይታ\n• ጎን እይታ\n• ውስጥ\n• ርቀት መለኪያ\n• ሞተር\n"
    else:
        # For rental, we didn't ask condition
        photo_prompt = "*ቀጣይ ደረጃ (አማራጭ)፡* የመኪናውን ፎቶዎች ይላኩ፡\n\n"
        photo_prompt += "• ፊት ለፊት እይታ\n• ጎን እይታ\n• ውስጥ\n• ዳሽቦርድ\n• ልዩ ባህሪያት\n"
