...
BROKER_PHONES = json.loads(os.getenv("BROKER_PHONES", '["+251913550415", "+251948002320", "+251911564697", "+251912391541"]'))
...
# Collect car make (for rental) - uses same handler as sale

# Collect model (for rental) - uses same handler as sale

# Collect year (for rental) - uses same handler as sale, then branches
@dp.message(CarForm.waiting_for_year)
...
async def get_warranty_needed(message: types.Message, state: FSMContext):
    valid_options = ["አዎ፣ ዋስትና አስፈላጊ ነው", "አይ፣ አስፈላጊ አይደለም"]
    if message.text not in valid_options:
        await message.answer("❌ እባክዎ ይምረጡ፡ 'አዎ፣ ዋስትና አስፈላጊ ነው' ወይም 'አይ፣ አስፈላጊ አይደለም'")
        return
...
async def get_rental_purpose(message: types.Message, state: FSMContext):
    valid_options = ["ለግል አጠቃቀም", "ለኢንተርፕራይዝ", "ለታክሲ አገልግሎት"]
    if message.text not in valid_options:
        await message.answer("❌ እባክዎ ይምረጡ፡ ለግል አጠቃቀም, ለኢንተርፕራይዝ, ወይም ለታክሲ አገልግሎት")
        return
