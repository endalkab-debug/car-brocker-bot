...
print(f"📞 እውቂያ: {', '.join(BROKER_PHONES[:2])}...")
print("="*60)
...
• ዋጋ: {ad_data.get('price', 'N/A')} ብር
• ሁኔታ: {ad_data.get('condition', 'N/A')[:100]}...
...
class CarForm(StatesGroup):
    ...
    waiting_for_rental_region = State()  # NEW: Added for rental region
...
# ====================
# RENTAL CONTINUATION (UPDATED AS PER PDF REQUIREMENTS)
# ====================
...
async def get_advanced_payment(message: types.Message, state: FSMContext):
    valid_options = ["አንድ ወር", "ሁለት ወር", "ሶስት ወር"]
    if message.text not in valid_options:
        await message.answer("❌ እባክዎ ይምረጡ፡ 'አንድ ወር', 'ሁለት ወር', ወይም 'ሶስት ወር'")
        return
...
async def get_warranty_needed(message: types.Message, state: FSMContext):
    valid_options = ["አዎ፣ አስፈላጊ ነው", "አይ፣ አስፈላጊ አይደለም"]
    if message.text not in valid_options:
        await message.answer("❌ እባክዎ ይምረጡ፡ 'አዎ፣ አስፈላጊ ነው' ወይም 'አይ፣ አስፈላጊ አይደለም'")
        return
...
async def get_rental_purpose(message: types.Message, state: FSMContext):
    valid_options = ["ለግል አጠቃቀም", "ለኢንተርፕራይዝ", "ለታክሲ አገልግሎት (Ride)", "ለጉብኝት (Tour)"]
    if message.text not in valid_options:
        await message.answer("❌ እባክዎ ከዚህ በታች ባሉት አማራጮች ይምረጡ:")
    ...
    await message.answer(
        "*9ኛ ደረጃ፡* መኪናው ለኪራይ የሚገኝበት ክልል (ከተማ) ያስገቡ፡\n"
        "ምሳሌ፡ Addis Ababa, Adama, Hawassa",
        parse_mode="Markdown"
    )
...
async def get_phone(message: types.Message, state: FSMContext):
    ...
    if car_type == 'sale':
        ...
        await message.answer(
            "*ቀጣይ ደረጃ፡* የመኪናውን ሁኔታ እና የኪራይ ውሎች ይግለጹ፡\n\n"
            "የሚካተት፡\n"
            "• ርቀት (ኪ.ሜ.)\n"
            "• ሁኔታ\n"
            "• ልዩ የኪራይ መስፈርቶች\n"
            "• የሚገኝበት ቀን\n"
            "• ማንኛውም ገደቦች\n\n"
            "*ምሳሌ፡* 'በደንብ የተጠገነ፣ 80,000 ኪ.ሜ.፣ ከሚጠባበቅ ሳምንት ይገኛል፣ በመኪና ውስጥ ማጨስ አይፈቀድም'",
            parse_mode="Markdown"
        )
...
• የኪራይ ዓላማ፡ {data.get('rental_purpose', '')}
• የሚገኝበት ክልል፡ {data.get('rental_region', '')}
...
*እገዛ ያስፈልግዎታል?* ከብሮከራችን ጋር ይገናኙ: {BROKER_PHONES[0] if BROKER_PHONES else '+251912345678'}
...
አዲስ አበባ መኪና ማዕከልን ስለታመኑ እናመሰግናለን! 🙏"""
        ...
        print(f"❌ {error_msg}")
        await message.answer(f"❌ ስህተት: {str(e)}\n\nእባክዎ እንደገና ይሞክሩ ወይም {BROKER_PHONES[0] if BROKER_PHONES else '+251912345678'} ይደውሉ")
...
• ቻናል: {ADMIN_CHANNEL}
• ብሮከር: {BROKER_NAME}
• ስልኮች: {', '.join(BROKER_PHONES[:2]) if BROKER_PHONES else '+251912345678'}
