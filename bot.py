...
async def start_command(message: types.Message):
    keyboard = types.ReplyKeyboardMarkup(
        keyboard=[
            [types.KeyboardButton(text="ð Sell Car"), 
             types.KeyboardButton(text="ð¢ Rent Car")],
            [types.KeyboardButton(text="ð Contact"),
             types.KeyboardButton(text="â¹ï¸ Help")]
        ],
    ...
    await message.answer(
        ...
        "Choose:\n"
        "â¢ ð Sell Car - Post for sale\n"
        "â¢ ð¢ Rent Car - Post for rental\n"
        "â¢ ð Contact - Get support\n\n"
        "Send /cancel anytime.",
        parse_mode="Markdown",
...
@dp.message(F.text.in_(["ð Sell Car", "ð¢ Rent Car"]))
async def start_ad_creation(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f"ð Creating {'Sale' if car_type == 'sale' else 'Rental'} Ad\n\n"
...
async def get_phone(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "ð¸ Send up to 5 photos of the car\n"
...
async def handle_photo(message: types.Message, state: FSMContext):
    ...
    if len(photos) < 5:
        ...
        await message.answer(f"â Photo added ({len(photos)}/5)\nSend /done when finished")
    else:
...
#{data['make']} #{data['model']} #CarSale #Ethiopia"""
        ...
        ad_text = f"""ð¢ *FOR RENT*

á¨áá¨á«á­ {data['make']} {data['model']} {data['year']}
*Daily Price:* {data['price']}

*Contact:* {data['phone']}

#{data['make']} #{data['model']} #CarRental #áªá«á­"""
...
async def stats_command(message: types.Message):
    if str(message.from_user.id) in ADMIN_IDS:
        ...
        await message.answer(
            f"ð *Statistics*\n\n"
            f"â¢ Total Ads: {total[0]}\n"
            f"â¢ Total Users: {users[0]}\n"
            f"â¢ Channel: {ADMIN_CHANNEL}",
            parse_mode="Markdown"
        )
...
async def broadcast_command(message: types.Message):
    if str(message.from_user.id) in ADMIN_IDS:
        ...
        if text:
            ...
            for user in users:
                try:
                    await bot.send_message(user[0], f"ð¢ Announcement:\n\n{text}")
...
*How to Post:*
1. Click "ð Sell Car" or "ð¢ Rent Car"
2. Follow the steps
