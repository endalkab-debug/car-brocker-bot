...
BROKER_PHONES = json.loads(os.getenv("BROKER_PHONES", '["+251912345678", "+251911223344", "+251922334455", "+251933445566"]'))
...
async def get_rental_purpose(message: types.Message, state: FSMContext):
    valid_options = ["ለግል አጠቃቀም", "ለድርጅት", "ለታክሲ አገልግሎት", "ለጉብኝት"]
    if message.text not in valid_options:
        await message.answer("❌ እባክዎ ይምረጡ፡ ለግል አጠቃቀም, ለድርጅት, ለታክሲ አገልግሎት, ወይም ለጉብኝት")
        return
    ...
    await message.answer(
        ...
        "ምሳሌ፡ አዲስ አበባ, አዳማ, ሐዋሳ, ባህር ዳር, መቀሌ",
        parse_mode="Markdown"
    )
    await state.set_state(CarForm.waiting_for_plate_region)
...
• ዋስትና፡ {data.get('rental_warranty', '')}
• የኪራይ ዓላማ፡ {data.get('rental_purpose', '')}
• ክልል፡ {data.get('plate_region', '')}
        ...
        if car_type == 'sale':
            ...
            await db.execute(
                ...
                (user_id, user_name, user_phone, make, model, year, plate_code, price, condition, car_type, photos,
                 rental_advanced, rental_warranty, rental_purpose, plate_region) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                (message.from_user.id, message.from_user.full_name, data['user_phone'], data['make'], data['model'], data['year'], 
                 ...
                 data.get('rental_purpose', ''), data.get('plate_region', ''))
            )
        await db.commit()
...
async def run_bot():
    ...
    try:
        ...
        print(f"⚠️ Webhook delete failed (not critical): {e}")
