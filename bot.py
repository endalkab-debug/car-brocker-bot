...
# ====================
# RENTAL CAR FLOW - AMHARIC (UPDATED)
# ====================
...
async def get_rental_purpose(message: types.Message, state: FSMContext):
    valid_options = ["ለግል አጠቃቀም", "ለድርጅት/ኢንተርፕራይዝ", "ለመንገደኛ መጓጓዣ (Ride)", "ለጉብኝት (Tour)"]
    if message.text not in valid_options:
        await message.answer("❌ እባክዎ ይምረጡ፡ ለግል አጠቃቀም, ለድርጅት/ኢንተርፕራይዝ, ለመንገደኛ መጓጓዣ (Ride), ወይም ለጉብኝት (Tour)")
        return
...
async def get_phone(message: types.Message, state: FSMContext):
    ...
    if car_type == 'sale':
        ...
        await message.answer(
            ...
            "• የሚገኝበት ቀን\n"
            "• ማንኛውም ገደቦች\n\n"
            "*ምሳሌ፡* 'በደንብ የተጠገነ፣ ከሚጠባበቅ ሳምንት ይገኛል፣ በመኪና ውስጥ ማጨስ አይፈቀድም'",
            parse_mode="Markdown"
        )
