...
Choose from the options below:"""
    ...
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="📞 Contact Brokers")]
        ],
        resize_keyboard=True
    )
...
# Collect plate code (sale only) - UPDATED WITH BUTTONS
...
# ====================
# RENTAL CONTINUATION (UPDATED WITH BUTTONS)
# ====================

# Collect plate code (rental only) - UPDATED WITH BUTTONS
...
# Collect advanced payment - UPDATED WITH BUTTONS
...
# Collect warranty needed - UPDATED WITH BUTTONS
...
# Collect rental purpose - UPDATED WITH BUTTONS
...
# Handle photos (common for both) - UPDATED TO HANDLE BUTTONS
...
async def handle_photo(message: types.Message, state: FSMContext):
    ...
    if len(photos) < 5:
        ...
        await message.answer(f"✅ Photo added ({len(photos)}/5)\n{remaining} more can be added. When finished, click '📸 Done' below.", reply_markup=get_photo_actions_keyboard())
    else:
        await message.answer("📸 Maximum 5 photos reached. To continue, click '📸 Done' below.", reply_markup=get_photo_actions_keyboard())

# Handle photo actions - NEW: BUTTON HANDLERS FOR PHOTOS
...
async def handle_photo_actions(message: types.Message, state: FSMContext):
    if message.text == "📸 Done - Finish Adding Photos":
        ...
        # If user sends text instead of photo, remind them
        await message.answer("Please send photos or use the buttons below:", reply_markup=get_photo_actions_keyboard())

# Show confirmation screen - NEW FUNCTION
async def show_confirmation(message: types.Message, state: FSMContext):
    data = await state.get_data()
    car_type = data.get('car_type', 'sale')
    photos = data.get('photos', [])
    
    # Format broker phones for preview
    broker_phones_formatted = "\n".join([f"• {phone}" for phone in BROKER_PHONES[:2]]) + "\n• ..."
...
📝 *Condition:*
{data['condition'][:200]}{'...' if len(data['condition']) > 200 else ''}

📞 *Contact (Brokers Only):*
{data['user_phone']}

📸 *Photos:* {len(photos)} photo(s) attached

*This ad will be posted on:* {ADMIN_CHANNEL}
*Brokers will contact you at:* {data['user_phone']}
...
• Advance: {data.get('rental_advanced', '')}
• Warranty: {data.get('rental_warranty', '')}
• Purpose: {data.get('rental_purpose', '')}
• Region: {data.get('rental_region', '')}
...
# Handle confirmation - NEW: CONFIRMATION HANDLER
...
# Process and post ad (updated to be called from confirmation)
